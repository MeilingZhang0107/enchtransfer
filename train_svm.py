...
import joblib                                   # NEW 
CHECKPOINT_DIR = "/scratch/s5511038/mustard_reproduce/MUStARD/mm"
os.makedirs(CHECKPOINT_DIR, exist_ok=True)      # NEW 

RESULT_FILE = "/scratch/s5511038/mustard_reproduce/output/mm/{}.json"
...
def train_speaker_dependent(config: Config, data: DataLoader, model_name: str) -> None:
    ...
    full_idx = list(range(len(data.data_input)))          
    full_train_in, full_train_out, _, _ = train_io(
        config=config,
        data=data,
        train_index=full_idx,
        test_index=full_idx                                     
    )
    full_clf = svm_train(config, full_train_in, full_train_out)

    full_path = os.path.join(CHECKPOINT_DIR, f"{model_name}.pkl")
    joblib.dump(full_clf, full_path)
    print(f"[✓] Full-data model saved to {full_path}")
