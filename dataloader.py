...
class DataLoader:
    DATA_PATH = "/scratch/s5511038/mustard_reproduce/MUStARD/data/sarcasm_data.json"
    AUDIO_PICKLE = "/scratch/s5511038/mustard_reproduce/MUStARD/data/context_audio_features.p"
    ...
    BERT_TARGET_EMBEDDINGS = "/scratch/s5511038/mustard_reproduce/MUStARD/data/bert-output_multi.jsonl"
    BERT_CONTEXT_EMBEDDINGS = "/scratch/s5511038/mustard_reproduce/MUStARD/data/bert-output_context_multi.jsonl"
    UTT_ID = 0
    CONTEXT_ID = 2
    SHOW_ID = 9
    UNK_TOKEN = "<UNK>"
    ...
    def __init__(self, config: config.Config) -> None:
        ...
        if config.use_target_video:
            video_features_file = h5py.File("/scratch/s5511038/mustard_reproduce/MUStARD/data/features/utterances_final/resnet_pool5.hdf5")
            context_video_features_file = h5py.File("/scratch/s5511038/mustard_reproduce/MUStARD/data/features/context_final/resnet_pool5.hdf5")
        else:
    ...
    def parse_data(self, dataset_dict: Mapping[str, Mapping[str, Any]],
        ...
        """Convert the dictionary data into lists."""
    ...
    def stratified_k_fold(self, splits: int = 5) -> None:
        """Prepare or load splits for K-fold cross-validation."""
    ...
    def full_dataset_vocab(self) -> Mapping[str, int]:
        """Return the vocabulary for the whole dataset for GloVe filtering."""
...
class DataHelper:
    ...
    def load_glove_model_for_current_split(self, use_context: bool = False) -> None:
        ...
        if os.path.exists(filename):
            ...
            # Filter GloVe dict to only words in the train set vocab
    ...
    def one_hot_output(self, mode: str, size: int) -> np.ndarray:
        """Return one-hot encoding of the output."""
        if mode == "train":
    ...
    def to_one_hot(data: Sequence[int], size: int) -> np.ndarray:
        """
        Return the one-hot label version of data.
        """
